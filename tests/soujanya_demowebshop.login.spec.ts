...
test('Demo Web Shop Login Testcase', async ({ browser }) => {
  ...
  await page.locator('#Email').fill('soujanya.26@gmail.com');
  ...
  const userEmailLink = page.locator('.account[href="/customer/info"]').first();
  await expect(userEmailLink).toHaveText('soujanya.26@gmail.com');
